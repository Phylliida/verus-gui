...
<file>              path relative to the repo root (e.g. src/size.rs)
[function_pattern]  optionally restrict verification to matching functions

examples:
  ./scripts/check-single-file.sh src/size.rs
  ./scripts/check-single-file.sh src/limits.rs
  ./scripts/check-single-file.sh src/layout.rs
  ./scripts/check-single-file.sh src/layout/proofs.rs lemma_resolve_bounds
EOF
  exit 0
fi
...
#   src/layout/proofs.rs  ->  layout::proofs
#   src/size.rs           ->  size
...
run_cargo_verus() {
  if command -v rustup >/dev/null 2>&1; then
    ...
    cd "$ROOT_DIR"
    cargo verus verify --manifest-path Cargo.toml -p verus-gui \
