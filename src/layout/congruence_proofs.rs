...
{
    match wrapper1 {
        WrapperWidget::Conditional { visible: false, child: c1 } => {
            if let Widget::Wrapper(WrapperWidget::Conditional { visible: false, child: c2 }) = w2 {
                lemma_layout_conditional_false_deep_any(lim1, lim2, c1, c2, fuel, fuel);
            }
        },
        WrapperWidget::Conditional { visible: true, child: c1 } => {
            if let Widget::Wrapper(WrapperWidget::Conditional { visible: true, child: c2 }) = w2 {
    ...
    decreases fuel, 2nat,
{
    ...
    match container {
        ...
        ContainerWidget::Flex { padding: p1, spacing: sp1, alignment: al, direction: dir, children: ch1 } => {
            if ch1.len() > 0 && fuel > 1 {
                lemma_container_full_depth_flex_dispatch(lim1, lim2, p1, sp1, al, dir, ch1, w2, fuel);
            }
        },
        _ => {
...
proof fn lemma_container_full_depth_row_dispatch<T: OrderedField>(
    ...
    requires
        limits_eqv(lim1, lim2),
        widget_eqv(Widget::Container(ContainerWidget::Row { padding: p1, spacing: sp1, alignment: al, children: ch1 }), w2, fuel),
        fuel > 1, ch1.len() > 0,
    ensures
        crate::diff::nodes_deeply_eqv(
            layout_widget(lim1, Widget::Container(ContainerWidget::Row { padding: p1, spacing: sp1, alignment: al, children: ch1 }), fuel),
            layout_widget(lim2, w2, fuel),
            min_children_congruence_depth(ch1, (fuel - 1) as nat, 0) + 1),
    decreases fuel, 1nat,
{
    if let Widget::Container(ContainerWidget::Row { padding: p2, spacing: sp2, children: ch2, .. }) = w2 {
        ...
        let s1 = Seq::new(cn1.len(), |i: int| cn1[i].size);
        let s2 = Seq::new(cn2.len(), |i: int| cn2[i].size);
        let ax = crate::layout::Axis::Horizontal;
        lemma_sub_congruence(lim1.max.cross_dim(ax), lim2.max.cross_dim(ax),
            p1.cross_padding(ax), p2.cross_padding(ax));
        lemma_linear_children_positions_congruence(p1, p2, sp1, sp2, al, s1, s2, ax,
            lim1.max.cross_dim(ax).sub(p1.cross_padding(ax)),
...
proof fn lemma_wrap_positions_eqv<T: OrderedField>(
    p1: Padding<T>, p2: Padding<T>,
    hs1: T, hs2: T, vs1: T, vs2: T,
    cs1: Seq<Size<T>>, cs2: Seq<Size<T>>,
    aw1: T, aw2: T,
    wl1: Node<T>, wl2: Node<T>,
)
    requires
        padding_eqv(p1, p2), hs1.eqv(hs2), vs1.eqv(vs2),
        sizes_eqv(cs1, cs2), aw1.eqv(aw2),
        cs1.len() > 0,
    ensures ({
        let wc1 = crate::layout::wrap::wrap_children(p1, hs1, vs1, cs1, aw1, 0);
        let wc2 = crate::layout::wrap::wrap_children(p2, hs2, vs2, cs2, aw2, 0);
        &&& wc1.len() == wc2.len()
        &&& forall|i: int| 0 <= i < wc1.len() ==> {
            &&& wc1[i].x.eqv(wc2[i].x)
            &&& wc1[i].y.eqv(wc2[i].y)
        }
    }),
{
    ...
    let wc1 = crate::layout::wrap::wrap_children(p1, hs1, vs1, cs1, aw1, 0);
    let wc2 = crate::layout::wrap::wrap_children(p2, hs2, vs2, cs2, aw2, 0);
    assert forall|i: int| 0 <= i < wc1.len() implies
        wc1[i].x.eqv(wc2[i].x) && wc1[i].y.eqv(wc2[i].y)
    by {
        crate::layout::wrap_proofs::lemma_wrap_children_element(p1, hs1, vs1, cs1, aw1, i as nat);
        crate::layout::wrap_proofs::lemma_wrap_children_element(p2, hs2, vs2, cs2, aw2, i as nat);
        ...
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
            cur2.x, cs1[i].width, cs2[i].width);
        ...
        if nb1 {
            ...
            verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                cur2.y, cur1.line_height, cur2.line_height);
            T::axiom_eqv_transitive(cur1.y.add(cur1.line_height),
                cur2.y.add(cur1.line_height), cur2.y.add(cur2.line_height));
            T::axiom_add_congruence_left(cur1.y.add(cur1.line_height),
                cur2.y.add(cur2.line_height), vs1);
            verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                cur2.y.add(cur2.line_height), vs1, vs2);
            T::axiom_eqv_transitive(
                cur1.y.add(cur1.line_height).add(vs1),
                cur2.y.add(cur2.line_height).add(vs1),
                cur2.y.add(cur2.line_height).add(vs2));
            T::axiom_add_congruence_left(p1.top, p2.top,
                cur1.y.add(cur1.line_height).add(vs1));
            verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                p2.top,
                cur1.y.add(cur1.line_height).add(vs1),
                cur2.y.add(cur2.line_height).add(vs2));
            T::axiom_eqv_transitive(
                p1.top.add(cur1.y.add(cur1.line_height).add(vs1)),
                p2.top.add(cur1.y.add(cur1.line_height).add(vs1)),
                p2.top.add(cur2.y.add(cur2.line_height).add(vs2)));
        } else {
            T::axiom_add_congruence_left(p1.left, p2.left, cur1.x);
            verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                p2.left, cur1.x, cur2.x);
            T::axiom_eqv_transitive(
                p1.left.add(cur1.x), p2.left.add(cur1.x), p2.left.add(cur2.x));
            T::axiom_add_congruence_left(p1.top, p2.top, cur1.y);
            verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                p2.top, cur1.y, cur2.y);
            T::axiom_eqv_transitive(
                p1.top.add(cur1.y), p2.top.add(cur1.y), p2.top.add(cur2.y));
        }
    };
}
...
proof fn lemma_listview_structural_bridge<T: OrderedField>(
    ...
    requires fuel > 0,
    ensures ({
        let cl = Limits { min: Size::zero_size(), max: Size::new(vp.width, vp.height) };
        let cs = crate::measure::measure_children(cl, children, (fuel - 1) as nat);
        ...
        && layout_widget(lim, Widget::Container(ContainerWidget::ListView {
            spacing: sp, scroll_y: sy, viewport: vp, children,
        }), fuel).children.len() == cn.len()
    }),
{
    reveal(crate::layout::listview::layout_listview_body);
}

/// Flex full-depth dispatch.
proof fn lemma_container_full_depth_flex_dispatch<T: OrderedField>(
    lim1: Limits<T>, lim2: Limits<T>,
    p1: Padding<T>, sp1: T, al: Alignment, dir: FlexDirection, ch1: Seq<FlexItem<T>>,
    w2: Widget<T>, fuel: nat,
)
    requires
        limits_eqv(lim1, lim2),
        widget_eqv(Widget::Container(ContainerWidget::Flex {
            padding: p1, spacing: sp1, alignment: al, direction: dir, children: ch1 }), w2, fuel),
        fuel > 1, ch1.len() > 0,
    ensures crate::diff::nodes_deeply_eqv(
        layout_widget(lim1, Widget::Container(ContainerWidget::Flex {
            padding: p1, spacing: sp1, alignment: al, direction: dir, children: ch1 }), fuel),
        layout_widget(lim2, w2, fuel),
        min_children_congruence_depth(
            Seq::new(ch1.len(), |i: int| ch1[i].child), (fuel - 1) as nat, 0) + 1),
    decreases fuel, 1nat,
{
    if let Widget::Container(ContainerWidget::Flex {
        padding: p2, spacing: sp2, children: ch2, ..
    }) = w2 {
        lemma_padding_horizontal_congruence(p1, p2);
        lemma_padding_vertical_congruence(p1, p2);
        lemma_shrink_congruence(lim1, lim2, p1.horizontal(), p2.horizontal(), p1.vertical(), p2.vertical());
        ...
        let ts1 = if ch1.len() > 0 { crate::layout::repeated_add(sp1, (ch1.len() - 1) as nat) } else { T::zero() };
        let axis = dir.axis();
        let am1 = inner1.max.main_dim(axis).sub(ts1);
        let cn1 = flex_linear_widget_child_nodes(inner1, ch1, w1s, tw1, am1, axis, (fuel - 1) as nat);
        let tw2 = sum_weights(w2s, w2s.len() as nat);
        let ts2 = if ch2.len() > 0 { crate::layout::repeated_add(sp2, (ch2.len() - 1) as nat) } else { T::zero() };
        let am2 = inner2.max.main_dim(axis).sub(ts2);
        let cn2 = flex_linear_widget_child_nodes(inner2, ch2, w2s, tw2, am2, axis, (fuel - 1) as nat);
        let wc1 = Seq::new(ch1.len(), |i: int| ch1[i].child);
        let rd = min_children_congruence_depth(wc1, (fuel - 1) as nat, 0);
        // ts (total spacing) congruence: repeated_add(sp, n) is eqv when sp eqv
        lemma_repeated_add_congruence(sp1, sp2, (ch1.len() - 1) as nat);
        // main_dim congruence (from limits_eqv + shrink_congruence → inner eqv)
        // am (available_main) congruence: main_dim.sub(ts) eqv
        lemma_sub_congruence(inner1.max.main_dim(axis), inner2.max.main_dim(axis), ts1, ts2);
        // IH: each flex child deeply eqv at rd
        assert forall|i: int| 0 <= i < cn1.len() implies
            crate::diff::nodes_deeply_eqv(#[trigger] cn1[i], cn2[i], rd)
        by {
            lemma_min_children_depth_le::<T>(wc1, (fuel - 1) as nat, 0, i);
            // flex_child_main_size eqv: w.div(tw).mul(av)
            verus_algebra::quadratic::lemma_div_congruence(w1s[i], w2s[i], tw1, tw2);
            T::axiom_mul_congruence_left(w1s[i].div(tw1), w2s[i].div(tw2), am1);
            verus_algebra::lemmas::ring_lemmas::lemma_mul_congruence_right::<T>(
                w2s[i].div(tw2), am1, am2);
            T::axiom_eqv_transitive(
                flex_child_main_size(w1s[i], tw1, am1),
                w2s[i].div(tw2).mul(am1),
                flex_child_main_size(w2s[i], tw2, am2));
            // IH call with per-child eqv limits
            match axis {
        ...
        lemma_sizes_eqv_from_deeply_eqv(cn1, cn2, rd);
        // Structural bridge + position congruence + merge+1
        match dir {
            FlexDirection::Column => {
                ...
                // Position congruence via flex_column_children_element + flex_column_child_y_congruence
                assert forall|i: int| 0 <= i < fl1.children.len() implies
                    fl1.children[i].x.eqv(fl2.children[i].x) && fl1.children[i].y.eqv(fl2.children[i].y)
                by {
                    crate::layout::flex_proofs::lemma_flex_column_children_element(
                        p1, sp1, al, w1s, ccs1, tw1, inner1.max.width, inner1.max.height, i as nat);
                    crate::layout::flex_proofs::lemma_flex_column_children_element(
                        p2, sp2, al, w2s, ccs2, tw2, inner2.max.width, inner2.max.height, i as nat);
                    lemma_align_offset_congruence(al, inner1.max.width, inner2.max.width, ccs1[i], ccs2[i]);
                    T::axiom_add_congruence_left(p1.left, p2.left, align_offset(al, inner1.max.width, ccs1[i]));
                    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                        p2.left, align_offset(al, inner1.max.width, ccs1[i]), align_offset(al, inner2.max.width, ccs2[i]));
                    T::axiom_eqv_transitive(
                        p1.left.add(align_offset(al, inner1.max.width, ccs1[i])),
                        p2.left.add(align_offset(al, inner1.max.width, ccs1[i])),
                        p2.left.add(align_offset(al, inner2.max.width, ccs2[i])));
                    lemma_flex_column_child_y_congruence(p1.top, p2.top, w1s, w2s, tw1, tw2,
                        inner1.max.height, inner2.max.height, sp1, sp2, i as nat);
                };
                lemma_layout_widget_node_congruence(lim1, lim2,
                    Widget::Container(ContainerWidget::Flex { padding: p1, spacing: sp1, alignment: al, direction: dir, children: ch1 }), w2, fuel);
            ...
            FlexDirection::Row => {
                ...
                crate::layout::flex_proofs::lemma_flex_row_children_len(p1, sp1, al, w1s, ccs1, tw1, inner1.max.width, inner1.max.height, 0);
                crate::layout::flex_proofs::lemma_flex_row_children_len(p2, sp2, al, w2s, ccs2, tw2, inner2.max.width, inner2.max.height, 0);
                assert forall|i: int| 0 <= i < fl1.children.len() implies
                    fl1.children[i].x.eqv(fl2.children[i].x) && fl1.children[i].y.eqv(fl2.children[i].y)
                by {
                    ...
                    crate::layout::flex_proofs::lemma_flex_row_children_element(
                        p2, sp2, al, w2s, ccs2, tw2, inner2.max.width, inner2.max.height, i as nat);
                    lemma_flex_row_child_x_congruence(p1.left, p2.left, w1s, w2s, tw1, tw2,
                        inner1.max.width, inner2.max.width, sp1, sp2, i as nat);
                    lemma_align_offset_congruence(al, inner1.max.height, inner2.max.height, ccs1[i], ccs2[i]);
                    T::axiom_add_congruence_left(p1.top, p2.top, align_offset(al, inner1.max.height, ccs1[i]));
                    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
                        p2.top, align_offset(al, inner1.max.height, ccs1[i]), align_offset(al, inner2.max.height, ccs2[i]));
                    T::axiom_eqv_transitive(
                        p1.top.add(align_offset(al, inner1.max.height, ccs1[i])),
                        p2.top.add(align_offset(al, inner1.max.height, ccs1[i])),
                        p2.top.add(align_offset(al, inner2.max.height, ccs2[i])));
                };
                lemma_layout_widget_node_congruence(lim1, lim2,
                    Widget::Container(ContainerWidget::Flex { padding: p1, spacing: sp1, alignment: al, direction: dir, children: ch1 }), w2, fuel);
                lemma_merge_layout_deep_congruence_plus_one(fl1, fl2, cn1, cn2, rd);
...
proof fn lemma_linear_layout_children_len<T: OrderedField>(
    ...
    ensures linear_layout(lim, pad, sp, al, cs, axis).children.len() == cs.len(),
{
    reveal(linear_layout);
    lemma_linear_children_len(pad, sp, al, cs, axis,
...
proof fn lemma_linear_children_len<T: OrderedField>(
    ...
    ensures linear_children(pad, sp, al, cs, axis, avail, index).len() == cs.len() - index,
    decreases cs.len() - index,
{
    if index >= cs.len() {
    } else {
        lemma_linear_children_len(pad, sp, al, cs, axis, avail, index + 1);
    }
}
...
proof fn lemma_merge_layout_children_structure<T: OrderedRing>(
    ...
    ensures
        merge_layout(layout, cn).children[i].x == layout.children[i].x,
        merge_layout(layout, cn).children[i].y == layout.children[i].y,
        merge_layout(layout, cn).children[i].size == cn[i].size,
        merge_layout(layout, cn).children[i].children == cn[i].children,
{
}
            ...
            fuel),
{
    ...
    by {
        lemma_linear_layout_children_len(lim1, p1, sp1, al, cs1, Axis::Vertical);
            ...
            fuel),
{
    ...
    assert forall|i: int| 0 <= i < n1.children.len() implies
        crate::diff::nodes_deeply_eqv(n1.children[i], n2.children[i], (fuel-1) as nat)
    by {
        lemma_linear_layout_children_len(lim1, p1, sp1, al, cs1, Axis::Horizontal);
        lemma_linear_layout_children_len(lim2, p2, sp2, al, cs2, Axis::Horizontal);
        lemma_merge_layout_children_structure(ll1, cn1, i);
        lemma_merge_layout_children_structure(ll2, cn2, i);
        assert(ll1.children == lc1);
        assert(ll2.children == lc2);
...
proof fn lemma_stack_full_deep<T: OrderedField>(
    ...
    requires
        ...
        forall|i: int| 0 <= i < cn1.len() ==>
            crate::diff::nodes_deeply_eqv(cn1[i], cn2[i], (fuel-1) as nat),
        node_eqv(
            layout_widget(lim1, Widget::Container(ContainerWidget::Stack {
                padding: p1, h_align: ha, v_align: va, children: ch1 }), fuel),
            layout_widget(lim2, Widget::Container(ContainerWidget::Stack {
                padding: p2, h_align: ha, v_align: va, children: ch2 }), fuel)),
    ensures
        crate::diff::nodes_deeply_eqv(
            layout_widget(lim1, Widget::Container(ContainerWidget::Stack {
                padding: p1, h_align: ha, v_align: va, children: ch1 }), fuel),
            ...
            fuel),
{
    lemma_stack_structural_bridge(lim1, p1, ha, va, ch1, fuel);
    lemma_stack_structural_bridge(lim2, p2, ha, va, ch2, fuel);
    let n1 = layout_widget(lim1, Widget::Container(ContainerWidget::Stack {
        padding: p1, h_align: ha, v_align: va, children: ch1 }), fuel);
    let n2 = layout_widget(lim2, Widget::Container(ContainerWidget::Stack {
        padding: p2, h_align: ha, v_align: va, children: ch2 }), fuel);
    let cs1 = Seq::new(cn1.len(), |i: int| cn1[i].size);
    let cs2 = Seq::new(cn2.len(), |i: int| cn2[i].size);
    assert(sizes_eqv(cs1, cs2));
...
proof fn lemma_wrap_full_deep<T: OrderedField>(
    ...
    requires
        ...
        forall|i: int| 0 <= i < cn1.len() ==>
            crate::diff::nodes_deeply_eqv(cn1[i], cn2[i], (fuel-1) as nat),
        node_eqv(
            layout_widget(lim1, Widget::Container(ContainerWidget::Wrap {
                padding: p1, h_spacing: hs1, v_spacing: vs1, children: ch1 }), fuel),
            layout_widget(lim2, Widget::Container(ContainerWidget::Wrap {
                padding: p2, h_spacing: hs2, v_spacing: vs2, children: ch2 }), fuel)),
    ensures
        crate::diff::nodes_deeply_eqv(
            layout_widget(lim1, Widget::Container(ContainerWidget::Wrap {
                padding: p1, h_spacing: hs1, v_spacing: vs1, children: ch1 }), fuel),
            ...
            fuel),
{
    lemma_wrap_structural_bridge(lim1, p1, hs1, vs1, ch1, fuel);
    lemma_wrap_structural_bridge(lim2, p2, hs2, vs2, ch2, fuel);
    let n1 = layout_widget(lim1, Widget::Container(ContainerWidget::Wrap {
        padding: p1, h_spacing: hs1, v_spacing: vs1, children: ch1 }), fuel);
    let n2 = layout_widget(lim2, Widget::Container(ContainerWidget::Wrap {
        padding: p2, h_spacing: hs2, v_spacing: vs2, children: ch2 }), fuel);
    let cs1 = Seq::new(cn1.len(), |i: int| cn1[i].size);
    let cs2 = Seq::new(cn2.len(), |i: int| cn2[i].size);
    assert(sizes_eqv(cs1, cs2));
...
proof fn lemma_absolute_children_positions_congruence<T: OrderedField>(
    ...
    requires
        ...
        index <= d1.len(),
    ensures ({
        let ac1 = crate::layout::absolute::absolute_children(pad1, d1, index);
        let ac2 = crate::layout::absolute::absolute_children(pad2, d2, index);
        ac1.len() == ac2.len()
        && forall|i: int| 0 <= i < ac1.len() ==> {
            &&& ac1[i].x.eqv(ac2[i].x)
            &&& ac1[i].y.eqv(ac2[i].y)
        }
    }),
    decreases d1.len() - index,
{
    ...
    if index >= d1.len() {
        ...
        T::axiom_eqv_transitive(
            pad1.left.add(d1[index as int].0), pad2.left.add(d1[index as int].0),
            pad2.left.add(d2[index as int].0));
        T::axiom_add_congruence_left(pad1.top, pad2.top, d1[index as int].1);
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
            pad2.top, d1[index as int].1, d2[index as int].1);
        T::axiom_eqv_transitive(
            pad1.top.add(d1[index as int].1), pad2.top.add(d1[index as int].1),
            pad2.top.add(d2[index as int].1));
        lemma_absolute_children_positions_congruence(pad1, pad2, d1, d2, index + 1);
        let ac1 = absolute_children(pad1, d1, index);
        let ac2 = absolute_children(pad2, d2, index);
        let rest1 = absolute_children(pad1, d1, index + 1);
        let rest2 = absolute_children(pad2, d2, index + 1);
        assert forall|i: int| 0 <= i < ac1.len() implies
            ac1[i].x.eqv(ac2[i].x) && ac1[i].y.eqv(ac2[i].y)
        by {
            if i == 0 {} else { assert(ac1[i] == rest1[i-1]); assert(ac2[i] == rest2[i-1]); }
        };
    }
}
...
proof fn lemma_sizedbox_full_deep<T: OrderedField>(
    ...
    requires
        limits_eqv(lim1, lim2), limits_eqv(il1, il2),
        fuel > 0,
        widget_eqv(*c1, *c2, (fuel-1) as nat),
        crate::diff::nodes_deeply_eqv(
            layout_widget(lim1.intersect(il1), *c1, (fuel-1) as nat),
            layout_widget(lim2.intersect(il2), *c2, (fuel-1) as nat),
            rec_depth),
    ensures
        crate::diff::nodes_deeply_eqv(
            layout_widget(lim1, Widget::Wrapper(WrapperWidget::SizedBox {
                inner_limits: il1, child: c1 }), fuel),
            layout_widget(lim2, Widget::Wrapper(WrapperWidget::SizedBox {
                inner_limits: il2, child: c2 }), fuel),
            rec_depth + 1),
{
    let w1 = Widget::Wrapper(WrapperWidget::SizedBox { inner_limits: il1, child: c1 });
    let w2 = Widget::Wrapper(WrapperWidget::SizedBox { inner_limits: il2, child: c2 });
    lemma_layout_widget_node_congruence(lim1, lim2, w1, w2, fuel);
    let rec1 = layout_widget(lim1.intersect(il1), *c1, (fuel-1) as nat);
    let rec2 = layout_widget(lim2.intersect(il2), *c2, (fuel-1) as nat);
    T::axiom_eqv_reflexive(T::zero());
    lemma_wrapper_child_deep_congruence(rec1, rec2,
        T::zero(), T::zero(), T::zero(), T::zero(), rec_depth);
    let n1 = layout_widget(lim1, w1, fuel);
    let n2 = layout_widget(lim2, w2, fuel);
    assert forall|i: int| 0 <= i < n1.children.len() implies
        crate::diff::nodes_deeply_eqv(n1.children[i], n2.children[i], rec_depth)
...
proof fn lemma_aspectratio_full_deep<T: OrderedField>(
    ...
    requires
        ...
        ({
            ...
            crate::diff::nodes_deeply_eqv(
                layout_widget(eff1, *c1, (fuel-1) as nat),
                layout_widget(eff2, *c2, (fuel-1) as nat),
                rec_depth)
        }),
    ensures
        crate::diff::nodes_deeply_eqv(
            layout_widget(lim1, Widget::Wrapper(WrapperWidget::AspectRatio {
                ratio: r1, child: c1 }), fuel),
            layout_widget(lim2, Widget::Wrapper(WrapperWidget::AspectRatio {
                ratio: r2, child: c2 }), fuel),
            rec_depth + 1),
{
    let w1 = Widget::Wrapper(WrapperWidget::AspectRatio { ratio: r1, child: c1 });
    let w2 = Widget::Wrapper(WrapperWidget::AspectRatio { ratio: r2, child: c2 });
    lemma_layout_widget_node_congruence(lim1, lim2, w1, w2, fuel);
    let h1 = lim1.max.width.div(r1);
    let eff1 = if h1.le(lim1.max.height) {
        Limits { min: lim1.min, max: Size::new(lim1.max.width, h1) }
    ...
    let rec1 = layout_widget(eff1, *c1, (fuel-1) as nat);
    let rec2 = layout_widget(eff2, *c2, (fuel-1) as nat);
    T::axiom_eqv_reflexive(T::zero());
    lemma_wrapper_child_deep_congruence(rec1, rec2,
        T::zero(), T::zero(), T::zero(), T::zero(), rec_depth);
    let n1 = layout_widget(lim1, w1, fuel);
...
proof fn lemma_conditional_true_full_deep<T: OrderedField>(
    ...
    ensures
        ...
        crate::diff::nodes_deeply_eqv(
            ...
            layout_widget(lim2, Widget::Wrapper(WrapperWidget::Conditional {
                visible: true, child: c2 }), fuel),
            rec_depth),
{
...
proof fn lemma_flex_main_sum_congruence<T: OrderedField>(
    ...
    requires
        w1.len() == w2.len(), n <= w1.len(),
        forall|i: int| 0 <= i < w1.len() ==> w1[i].eqv(w2[i]),
        tw1.eqv(tw2), av1.eqv(av2),
        !tw1.eqv(T::zero()),
    ensures
        crate::layout::flex::flex_main_sum(w1, tw1, av1, n)
            .eqv(crate::layout::flex::flex_main_sum(w2, tw2, av2, n)),
    decreases n,
{
    use crate::layout::flex::{flex_main_sum, flex_child_main_size};
    if n == 0 {
        T::axiom_eqv_reflexive(T::zero());
    } else {
        ...
        T::axiom_add_congruence_left(
            ...
            flex_child_main_size(w1[(n-1) as int], tw1, av1));
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
            flex_main_sum(w2, tw2, av2, (n-1) as nat),
            flex_child_main_size(w1[(n-1) as int], tw1, av1),
            flex_child_main_size(w2[(n-1) as int], tw2, av2));
        T::axiom_eqv_transitive(
            flex_main_sum(w1, tw1, av1, (n-1) as nat).add(flex_child_main_size(w1[(n-1) as int], tw1, av1)),
            flex_main_sum(w2, tw2, av2, (n-1) as nat).add(flex_child_main_size(w1[(n-1) as int], tw1, av1)),
            flex_main_sum(w2, tw2, av2, (n-1) as nat).add(flex_child_main_size(w2[(n-1) as int], tw2, av2)));
    }
}
...
proof fn lemma_flex_column_child_y_congruence<T: OrderedField>(
    ...
    requires
        pt1.eqv(pt2),
        w1.len() == w2.len(), k <= w1.len(),
        forall|i: int| 0 <= i < w1.len() ==> w1[i].eqv(w2[i]),
        tw1.eqv(tw2), ah1.eqv(ah2), sp1.eqv(sp2),
        !tw1.eqv(T::zero()),
    ensures
        crate::layout::flex::flex_column_child_y(pt1, w1, tw1, ah1, sp1, k)
            .eqv(crate::layout::flex::flex_column_child_y(pt2, w2, tw2, ah2, sp2, k)),
{
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        pt2,
        crate::layout::flex::flex_main_sum(w1, tw1, ah1, k),
        crate::layout::flex::flex_main_sum(w2, tw2, ah2, k));
    T::axiom_eqv_transitive(
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        mid2, repeated_add(sp1, k), repeated_add(sp2, k));
    T::axiom_eqv_transitive(
        mid1.add(repeated_add(sp1, k)), mid2.add(repeated_add(sp1, k)),
...
proof fn lemma_flex_row_child_x_congruence<T: OrderedField>(
    ...
    requires
        ...
        w1.len() == w2.len(), k <= w1.len(),
        forall|i: int| 0 <= i < w1.len() ==> w1[i].eqv(w2[i]),
        tw1.eqv(tw2), aw1.eqv(aw2), sp1.eqv(sp2),
        !tw1.eqv(T::zero()),
    ensures
            ...
            .eqv(crate::layout::flex::flex_row_child_x(pl2, w2, tw2, aw2, sp2, k)),
{
    ...
    T::axiom_add_congruence_left(pl1, pl2,
        crate::layout::flex::flex_main_sum(w1, tw1, aw1, k));
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        pl2,
        crate::layout::flex::flex_main_sum(w1, tw1, aw1, k),
        crate::layout::flex::flex_main_sum(w2, tw2, aw2, k));
    T::axiom_eqv_transitive(
        pl1.add(crate::layout::flex::flex_main_sum(w1, tw1, aw1, k)),
        pl2.add(crate::layout::flex::flex_main_sum(w1, tw1, aw1, k)),
        pl2.add(crate::layout::flex::flex_main_sum(w2, tw2, aw2, k)));
    let mid1 = pl1.add(crate::layout::flex::flex_main_sum(w1, tw1, aw1, k));
    let mid2 = pl2.add(crate::layout::flex::flex_main_sum(w2, tw2, aw2, k));
    T::axiom_add_congruence_left(mid1, mid2, repeated_add(sp1, k));
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        mid2, repeated_add(sp1, k), repeated_add(sp2, k));
    T::axiom_eqv_transitive(
...
proof fn lemma_flex_column_full_deep<T: OrderedField>(
    ...
    sp1: T, sp2: T, al: Alignment,
    w1s: Seq<T>, w2s: Seq<T>,
    ch1: Seq<FlexItem<T>>, ch2: Seq<FlexItem<T>>,
    fuel: nat,
    cn1: Seq<Node<T>>, cn2: Seq<Node<T>>,
)
    requires
        ...
        padding_eqv(p1, p2), sp1.eqv(sp2),
        ch1.len() == ch2.len(), fuel > 1,
        w1s.len() == ch1.len(), w2s.len() == ch2.len(),
        forall|i: int| 0 <= i < ch1.len() ==> w1s[i].eqv(w2s[i]),
        !sum_weights(w1s, w1s.len() as nat).eqv(T::zero()),
        cn1.len() == ch1.len(), cn2.len() == ch2.len(),
        forall|i: int| 0 <= i < cn1.len() ==>
            crate::diff::nodes_deeply_eqv(cn1[i], cn2[i], (fuel-1) as nat),
        node_eqv(
            merge_layout(
                flex_column_layout(lim1, p1, sp1, al, w1s,
                    Seq::new(cn1.len(), |i: int| cn1[i].size.width)), cn1),
            merge_layout(
                flex_column_layout(lim2, p2, sp2, al, w2s,
                    Seq::new(cn2.len(), |i: int| cn2[i].size.width)), cn2)),
    ensures
        crate::diff::nodes_deeply_eqv(
            merge_layout(
                flex_column_layout(lim1, p1, sp1, al, w1s,
            ...
            fuel),
{
    ...
    crate::layout::flex_proofs::lemma_flex_column_children_len(
        ...
        lim1.max.width.sub(p1.horizontal()),
        lim1.max.height.sub(p1.vertical()).sub(
            if w1s.len() > 0 { repeated_add(sp1, (w1s.len()-1) as nat) } else { T::zero() }),
        0);
    crate::layout::flex_proofs::lemma_flex_column_children_len(
        p2, sp2, al, w2s, ccs2,
        sum_weights(w2s, w2s.len() as nat),
        lim2.max.width.sub(p2.horizontal()),
        lim2.max.height.sub(p2.vertical()).sub(
    ...
    by {
        ...
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
            p2.left, align_offset(al, aw1, ccs1[i]), align_offset(al, aw2, ccs2[i]));
        T::axiom_eqv_transitive(
        ...
        assert(ah1.eqv(ah2)) by {
            ...
            lemma_sub_congruence(lim1.max.height, lim2.max.height, p1.vertical(), p2.vertical());
            if w1s.len() > 0 {
                lemma_repeated_add_congruence(sp1, sp2, (w1s.len()-1) as nat);
                lemma_sub_congruence(base1, base2,
                    repeated_add(sp1, (w1s.len()-1) as nat),
                    repeated_add(sp2, (w2s.len()-1) as nat));
            } else {
                T::axiom_eqv_reflexive(T::zero());
                lemma_sub_congruence(base1, base2, T::zero(), T::zero());
            }
        };
        lemma_flex_column_child_y_congruence(
            p1.top, p2.top, w1s, w2s, tw1, tw2, ah1, ah2, sp1, sp2, i as nat);
        lemma_wrapper_child_deep_congruence(
            cn1[i], cn2[i], n1.children[i].x, n2.children[i].x,
            n1.children[i].y, n2.children[i].y, (fuel-1) as nat);
    };
...
proof fn lemma_flex_row_full_deep<T: OrderedField>(
    ...
    requires
        ...
        node_eqv(
            merge_layout(
                flex_row_layout(lim1, p1, sp1, al, w1s,
                    Seq::new(cn1.len(), |i: int| cn1[i].size.height)), cn1),
            merge_layout(
                flex_row_layout(lim2, p2, sp2, al, w2s,
                    Seq::new(cn2.len(), |i: int| cn2[i].size.height)), cn2)),
    ensures
        crate::diff::nodes_deeply_eqv(
            merge_layout(
                flex_row_layout(lim1, p1, sp1, al, w1s,
            ...
            fuel),
{
    ...
    crate::layout::flex_proofs::lemma_flex_row_children_len(
        ...
        ah1, 0);
    crate::layout::flex_proofs::lemma_flex_row_children_len(
        p2, sp2, al, w2s, ccs2, tw2,
        lim2.max.width.sub(p2.horizontal()).sub(
            if w2s.len() > 0 { repeated_add(sp2, (w2s.len()-1) as nat) } else { T::zero() }),
        ah2, 0);
    ...
    by {
        lemma_merge_layout_children_structure(fl1, cn1, i);
        lemma_merge_layout_children_structure(fl2, cn2, i);
        let aw1 = lim1.max.width.sub(p1.horizontal()).sub(
            if w1s.len() > 0 { repeated_add(sp1, (w1s.len()-1) as nat) } else { T::zero() });
        let aw2 = lim2.max.width.sub(p2.horizontal()).sub(
            if w2s.len() > 0 { repeated_add(sp2, (w2s.len()-1) as nat) } else { T::zero() });
        crate::layout::flex_proofs::lemma_flex_row_children_element(
            p1, sp1, al, w1s, ccs1, tw1, aw1, ah1, i as nat);
        crate::layout::flex_proofs::lemma_flex_row_children_element(
        ...
        verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
            p2.top, align_offset(al, ah1, ccs1[i]), align_offset(al, ah2, ccs2[i]));
        T::axiom_eqv_transitive(
            p1.top.add(align_offset(al, ah1, ccs1[i])),
            p2.top.add(align_offset(al, ah1, ccs1[i])),
            p2.top.add(align_offset(al, ah2, ccs2[i])));
        lemma_wrapper_child_deep_congruence(
            cn1[i], cn2[i], n1.children[i].x, n2.children[i].x,
            n1.children[i].y, n2.children[i].y, (fuel-1) as nat);
    };
...
proof fn lemma_grid_cell_x_congruence<T: OrderedRing>(
    ...
    requires pl1.eqv(pl2), sizes_eqv(cw1, cw2), hs1.eqv(hs2), col <= cw1.len(),
    ensures
            ...
            .eqv(crate::layout::grid::grid_cell_x(pl2, cw2, hs2, col)),
{
    ...
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        mid2, repeated_add(hs1, col), repeated_add(hs2, col));
    T::axiom_eqv_transitive(
        mid1.add(repeated_add(hs1, col)), mid2.add(repeated_add(hs1, col)),
...
proof fn lemma_grid_cell_y_congruence<T: OrderedRing>(
    ...
    requires pt1.eqv(pt2), sizes_eqv(rh1, rh2), vs1.eqv(vs2), row <= rh1.len(),
    ensures
        crate::layout::grid::grid_cell_y(pt1, rh1, vs1, row)
            .eqv(crate::layout::grid::grid_cell_y(pt2, rh2, vs2, row)),
{
    lemma_sum_heights_congruence(rh1, rh2, row);
    lemma_repeated_add_congruence(vs1, vs2, row);
    T::axiom_add_congruence_left(pt1, pt2, sum_heights(rh1, row));
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        pt2, sum_heights(rh1, row), sum_heights(rh2, row));
    T::axiom_eqv_transitive(
        pt1.add(sum_heights(rh1, row)), pt2.add(sum_heights(rh1, row)),
        pt2.add(sum_heights(rh2, row)));
    let mid1 = pt1.add(sum_heights(rh1, row));
    let mid2 = pt2.add(sum_heights(rh2, row));
    T::axiom_add_congruence_left(mid1, mid2, repeated_add(vs1, row));
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(
        mid2, repeated_add(vs1, row), repeated_add(vs2, row));
    T::axiom_eqv_transitive(
        mid1.add(repeated_add(vs1, row)), mid2.add(repeated_add(vs1, row)),
...
proof fn lemma_grid_col_width_congruence<T: OrderedRing>(
    ...
    requires sizes_eqv(cw1, cw2), col < cw1.len(),
    ensures
...
proof fn lemma_grid_row_height_congruence<T: OrderedRing>(
    rh1: Seq<Size<T>>, rh2: Seq<Size<T>>, row: nat,
)
    requires sizes_eqv(rh1, rh2), row < rh1.len(),
    ensures
...
proof fn lemma_grid_child_position_congruence<T: OrderedField>(
    ...
    ensures ({
        let gc1 = crate::layout::grid::grid_child(p1, cw1, rh1, hs1, vs1, ha, va, row, col, cs1);
        let gc2 = crate::layout::grid::grid_child(p2, cw2, rh2, hs2, vs2, ha, va, row, col, cs2);
    ...
    }),
{
    ...
    lemma_align_offset_congruence(ha,
        crate::layout::grid::grid_col_width(cw1, col),
        crate::layout::grid::grid_col_width(cw2, col),
        cs1.width, cs2.width);
    let cx1 = crate::layout::grid::grid_cell_x(p1.left, cw1, hs1, col);
    let cx2 = crate::layout::grid::grid_cell_x(p2.left, cw2, hs2, col);
    let ao_x1 = align_offset(ha, crate::layout::grid::grid_col_width(cw1, col), cs1.width);
    let ao_x2 = align_offset(ha, crate::layout::grid::grid_col_width(cw2, col), cs2.width);
    ...
    lemma_align_offset_congruence(va,
        crate::layout::grid::grid_row_height(rh1, row),
        crate::layout::grid::grid_row_height(rh2, row),
        cs1.height, cs2.height);
    let cy1 = crate::layout::grid::grid_cell_y(p1.top, rh1, vs1, row);
    let cy2 = crate::layout::grid::grid_cell_y(p2.top, rh2, vs2, row);
    let ao_y1 = align_offset(va, crate::layout::grid::grid_row_height(rh1, row), cs1.height);
    let ao_y2 = align_offset(va, crate::layout::grid::grid_row_height(rh2, row), cs2.height);
    T::axiom_add_congruence_left(cy1, cy2, ao_y1);
    verus_algebra::lemmas::additive_group_lemmas::lemma_add_congruence_right::<T>(cy2, ao_y1, ao_y2);
...
proof fn lemma_grid_structural_bridge<T: OrderedField>(
    ...
    ensures ({
        let inner = lim.shrink(pad.horizontal(), pad.vertical());
        let cn = grid_widget_child_nodes(inner, cw, rh, children, cw.len(), (fuel-1) as nat);
        let ncols = cw.len();
        let nrows = rh.len();
        let cs2d = Seq::new(nrows, |r: int| Seq::new(ncols, |c: int| cn[(r * ncols as int + c)].size));
        layout_widget(lim, Widget::Container(ContainerWidget::Grid {
            padding: pad, h_spacing: hs, v_spacing: vs, h_align: ha, v_align: va,
            col_widths: cw, row_heights: rh, children,
        }), fuel) == merge_layout(
            crate::layout::grid::grid_layout(lim, pad, hs, vs, ha, va, cw, rh, cs2d), cn)
    }),
{
    reveal(crate::layout::grid::grid_layout);
    let inner = lim.shrink(pad.horizontal(), pad.vertical());
    let cn = grid_widget_child_nodes(inner, cw, rh, children, cw.len(), (fuel-1) as nat);
    let g = ContainerWidget::Grid {
        padding: pad, h_spacing: hs, v_spacing: vs, h_align: ha, v_align: va,
        col_widths: cw, row_heights: rh, children };
    let w = Widget::Container(g);
    assert(layout_widget(lim, w, fuel) == layout_container(lim, g, fuel));
    let ncols = cw.len();
    let nrows = rh.len();
    let cs2d = Seq::new(nrows, |r: int| Seq::new(ncols, |c: int| cn[(r * ncols as int + c)].size));
    assert(layout_container(lim, g, fuel)
        == layout_grid_body(lim, pad, hs, vs, ha, va, cw, rh, cn));
    assert(layout_grid_body(lim, pad, hs, vs, ha, va, cw, rh, cn)
